...
| Page | URL | Rôle |
| --- | --- | --- |
| Accueil | `/` | Landing de l'appli : survol 3D de l'île au scroll, fonctionnalités, modules |
| La Réunion en chiffres | `/chiffres.html` | Tableau de bord bento en temps réel (compteurs, odomètres, animations) |
| La carte du 974 | `/carte.html` | Carte interactive open data (sentiers GR, toilettes, eau, équipements) |
...
extension (`/chiffres` comme `/chiffres.html`).
...
- `robots.txt` et `sitemap.xml` à la racine, pointés sur `https://alerte.re/`.
- Chaque page porte title/description uniques, Open Graph, `theme-color` et des
  données structurées JSON-LD (Organization, SoftwareApplication, Dataset).
...
`data/*.geojson` (~390 Ko) : extraits d'OpenStreetMap (licence ODbL) via l'API
Overpass — 42 itinéraires `route=hiking` (dont GR R1/R2/R3, simplifiés
...
Leaflet 1.9.4, Leaflet.markercluster et Three sont vendorisés dans `vendor/`.
...
sont des attributs `data-rate-year` dans `chiffres.html`.

## Design

Design system « Modernist » (Archivo, accent #ec3013), assoupli en grille bento
arrondie, calibré mobile-first. Le bundle de maquettes d'origine
(`splashscreen-alerte-r-union/`) et la web radio Onde Locale ont été retirés du
dépôt — ils restent disponibles dans l'historique git.
